	...
	public void actionPerformed(ActionEvent e) {
		...
		if(!this.vue.nameFieldIsEmpty() && !this.vue.firstNameFieldIsEmpty()){
			...
			if (this.vue.isPro()){
				...
				if(this.carnet.ajout(pro)) good = true;
				...
				if(this.carnet.ajout(part)) good = true;
			}
			this.frame.majListe();
			this.frame.majJList();
			this.frame.details();
			if (good) this.frame.setTextPanelDown("Contact ajouté avec succès",good);
			else this.frame.setTextPanelDown("Impossible d'ajouter ce contact",good);
		}
	}	
}